...
export default function TVDetailPage() {
  ...
  const [recentReviews, setRecentReviews] = useState<TVDetail["community"]["recentReviews"]>([]);
  ...
  useEffect(() => { if (show) setRecentReviews(show.community.recentReviews || []); }, [show]);
  useEffect(() => {
    ...
    fetch(`${apiBase}/api/reviews/me`, { headers: { Authorization: `Bearer ${accessToken}` } })
      ...
      .then((data) => {
        const match = data?.data?.find((r: any) => r.media?.tmdbId === show.id && r.media?.type === "TV_SHOW");
  ...
  async function handleSubmitReview() {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/reviews`, {
        ...
        body: JSON.stringify({ tmdbId: show?.id, type: "TV_SHOW", title: reviewTitle, body: reviewBody }),
      });
      if (!res.ok) throw new Error("Failed to submit review");
      const created = await res.json();
      setRecentReviews((r) => [created, ...r]);
      if (show) setShow({ ...show, community: { ...show.community, totalReviews: show.community.totalReviews + 1 } });
      setReviewTitle("");
      setReviewBody("");
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 16px" }}>
        ...
        <div style={{ display: "flex", gap: 32, marginTop: 24, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 240 }}>
            <p style={{ margin: 0, color: "var(--accent)", fontSize: 12, textTransform: "uppercase", letterSpacing: "0.12em" }}>TV Show</p>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ padding: "4px 12px", borderRadius: 9999, background: "var(--surface-2)", color: "var(--text-muted)", fontSize: 13 }}>
                {new Date(show.first_air_date).toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" })}
              </span>
              <span style={{ padding: "4px 12px", borderRadius: 9999, background: "var(--surface-2)", color: "var(--text-muted)", fontSize: 13 }}>
                {show.status}
              </span>
            ...
            <p style={{ margin: 0, color: "var(--text-muted)", lineHeight: 1.75, fontSize: 15 }}>{show.overview}</p>
          </div>
        </div>
        ...
        <div style={{ marginTop: 40, borderTop: "1px solid var(--border)", paddingTop: 32 }}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 32 }}>
            {[
              { label: "Avg Rating", value: show.community.avgRating != null ? show.community.avgRating.toFixed(1) : "—" },
              { label: "Total Ratings", value: show.community.totalRatings },
              { label: "Total Reviews", value: show.community.totalReviews },
            ].map(({ label, value }) => (
